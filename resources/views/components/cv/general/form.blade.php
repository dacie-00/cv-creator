...
<x-input-label for="phone_number">
    {{ __('Phone Number') }}<x-required-star/>
</x-input-label>
<x-text-input name="phone_number" required maxlength="30" value="{{ old('phone_number') ?? $cv->phone_number }}"></x-text-input>
<x-input-error :messages="$errors->get('phone_number')" class="mt-2"/>

<x-input-label for="address">
    {{ __('Address') }}<x-required-star/>
</x-input-label>
